...
import { Menu, X, User as UserIcon, LogOut, Wallet, Shield } from "lucide-react";
        ...
        <div className="md:hidden absolute top-full left-0 right-0 bg-background/95 backdrop-blur-xl border-b border-border p-4 flex flex-col gap-4">
          ...
          {user ? (
            <>
              ...
              {user.role === "admin" && (
                <Link href="/admin" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 p-2 text-white"><Shield size={18} /> Админ-панель</Link>
              )}
