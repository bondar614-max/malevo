...
router.get("/admin/dashboard", async (req, res) => {
  ...
  const [
    ...
    db.select({ status: ordersTable.status, count: sql<number>`count(*)` })
      .from(ordersTable)
      .where(orderWhere)
  ...
  const days = range.period === "all"
    ? Array.from(orderDaily.keys()).sort()
    : eachDay(range.from, range.to);
  const grossOrders = Number(ordersSummaryRows[0]?.grossOrders ?? 0);
  const ordersSuccess = Number(ordersSummaryRows[0]?.success ?? 0);

  res.json({
    period: { key: range.period, from: range.from ? range.from.toISOString() : null, to: range.to ? range.to.toISOString() : null },
    summary: {
      ...
      paymentsCount: ordersSuccess,
      paymentsRevenue: grossOrders,
      averagePayment: ordersSuccess ? grossOrders / ordersSuccess : 0,
    },
    statuses: statusRows.map((r) => ({ status: r.status, count: Number(r.count) })),
    topItems: topRows.map((r) => ({ key: r.key, label: r.label, count: Number(r.count), revenue: Number(r.revenue), success: Number(r.success) })),
    daily: days.map((day) => ({
      day,
      ...
      paymentsRevenue: Number(orderDaily.get(day)?.revenue ?? 0),
    })),
